...
/*
 	...
 	클래스 내부에 여러 개의 동기화 메소드가 존재할 경우: 모두 스레드가 접근하는 키가 this
 	여러 개의 동기화 메소드 내에서 멀티 스레드를 사용하고자 할 경우: 동기화 블록을 생성 후 키를 다르게 설정.
...
class MyData{//공유 객체: 여러 스레드가 접근
	...
	void bcd() {	//여러 스레드가 접근할 때 동시성 문제를 방지하기 위하여 동기화 처리
		synchronized (new A()) {		//스레드가 접근할 별도의 키를 생성
			for(int i=0;i<3;i++) {
