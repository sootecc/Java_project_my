...
public class HashTableMethod {
	...
	public static void main(String[] args) {
		...
		hTable1.put(1, "가나다");
		hTable1.put(3, "다라마");
		hTable1.put(3, "가가가");		//중복된 키에 값을 넣을 경우 기존 값이 수정 
		...
		hTable2.replace(4, "라라라");	//key 4는 존재하지 않기 때문에 작동되지 않는다. 예외발생은 아님
		System.out.println(hTable2);
		...
		hTable2.replace(2, "다다다", "라라라");	//key와 value가 정확히 일치해야 수정된다.
		System.out.println(hTable2);
		...
		System.out.println(hTable2.get(1));	//1은 인덱스가 아니라 key
		...
		System.out.println(hTable2.containsValue("나나나"));		//t
		System.out.println(hTable2.containsValue("다다다"));		//f
		System.out.println(hTable2.containsValue("하하하"));		//f
